...
## 빠른 시작
### API 개요
리더 등록, 유예 기간(grace period) 관리, 메모리 배리어가 필요 없습니다.<br>
```cpp
storage = new_storage;      // 업데이트 예시 (std::shared_ptr<T> new_storage)
auto data = storage.load(); // guard 객체를 반환
```
*참고1: 전통적인 RCU와 달리, cppurcu는 이전 데이터를 직접 회수하지 않고
`std::shared_ptr`에 회수를 위임합니다.
이전 객체에 대한 `std::shared_ptr` 참조는 스코프 내에서 첫 번째 `load()` 호출 시 해제됩니다.
따라서 모든 `std::shared_ptr` 인스턴스에 대한 모든 참조가 해제될 때 메모리가 회수됩니다.*<br>
...
- 일반적으로 `storage`를 전역, 정적, 또는 수명이 긴 멤버 변수로 선언하세요
...
// 모두 로드 – 동일한 스냅샷 시점을 유지
const auto &[a, b, c] = cppurcu::load(storageA, storageB, storageC);

// 이 스코프 내의 모든 읽기는 일관된 데이터를 봅니다,
// 다른 스레드에서 업데이트가 발생하더라도.
a->lookup(...);
b->query(...);
c->find(...);
```
특정 스코프 내에서 일관된 스냅샷이 필요한 경우, 다음과 같이 코드를 작성할 수 있습니다:
```cpp
#include <cppurcu/cppurcu.h>
...........
{
  ...
  // pack은 storageA, B, C의 일관된 스냅샷을 유지합니다.
  my_class.calculate(...);
  .....
}
```
...
3. 변경되었으면: 캐시의 버전, shared_ptr, raw 포인터를 업데이트 (느린 경로)
<br>reclaimer_thread 활성화 시: 이전 값을 리클레이머 큐에 푸시

### 리클레이머 스레드 (선택 사항)

활성화되면, reclaimer_thread가 백그라운드에서 객체 소멸을 처리합니다:
1. storage::load()가 데이터 업데이트 시 이전 shared_ptr을 리클레임 큐에 푸시
