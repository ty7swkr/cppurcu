...
RCU（Read-Copy-Update）是一种用于读取密集型工作负载的同步技术，在这种场景下 `std::mutex`、`std::shared_mutex` 或自旋锁会成为可扩展性瓶颈；它让写入器发布更新后的版本，同时让读取器无需加锁即可读取数据。

在读取密集型工作负载中，使用 RCU 算法通常可以在读取侧相比基于锁的同步获得数倍到 10 倍以上的性能提升。

<br>

## 主要特性

> 一个易用的现代 C++ 库，提供防误用的快照隔离，并且比 `std::mutex` 更简单。
...
- `storage` 应声明为全局变量、静态变量或长生命周期的成员变量。
...
// 各 storage 各自的快照，在作用域内一致保持
const auto &[a, b, c] = cppurcu::load(storageA, storageB, storageC);

// 对于每个 storage，此作用域内的所有读取都会看到相同的快照，
// 即使其他线程发生更新。
a->lookup(...);
b->query(...);
c->find(...);
...
如果需要在特定作用域内一致保持各 storage 的快照，可以这样编写代码：
...
3. 如果已更改：更新缓存中的版本、shared_ptr 和原始指针（慢速路径）<br>
读取路径不会执行回收队列操作
...
1. storage::update() 在数据更新时将被替换的 shared_ptr 推入回收队列
